Given %r/^I upload a new report with different comments$/ do
...
Then %r/^the testcase "([^"]*)" should have the new comment$/ do |testcase|
...
And %r/^the testcase "([^"]*)" should have the comment from the previous report$/ do |testcase|
  session = MeegoTestSession.last
  tc = session.meego_test_cases.find_by_name(testcase)
