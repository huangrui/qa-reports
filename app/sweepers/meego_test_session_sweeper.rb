class MeegoTestSessionSweeper < ActionController::Caching::Sweeper
  ...
  def expire_index_for(test_session)
    ...
    expire_paging_action :controller => "index", :action => "filtered_list", :release_version => test_session.release_version, :target => test_session.target, :testtype => test_session.testtype, :hardware => test_session.hardware
