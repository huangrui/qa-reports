class Index
  ...
  def self.find_by_build_release(release, scope)
    { :profiles => Profile.find_by_sql("
        SELECT DISTINCT profiles.name AS profile, reports.build_id, reports.testset, reports.product AS name
        FROM profiles
        LEFT JOIN meego_test_sessions AS reports ON profiles.id = reports.profile_id AND
          reports.release_id = #{release.id} AND
          reports.published  = TRUE AND
          reports.tested_at  > '#{scope == 'all' ? 0 : CUT_OFF_LIMIT.days.ago}'
        ORDER BY profiles.sort_order ASC, build_id DESC, testset, product
      ").group_by(&:profile).map do |profile, build_ids|
        {
          :name       => profile,
          :url        => "/#{release.name}/#{profile}",
          :build_ids  => build_ids.select{|bd| bd.build_id.present?}.group_by(&:build_id).map do |build_id, testsets|
            {
              :name     => "Build " + build_id + ':',
              ...
              :testsets => testsets.select{|ts| ts.testset.present?}.group_by(&:testset).map do |testset, products|
                  {
                    :name           => testset + ':',
                    :url            => "/#{release.name}/build/#{build_id}/#{profile}/#{testset}",
                    :products       => products.map do |product|
                      {
                        :name => product.name,
                        :url  => "/#{release.name}/build/#{build_id}/#{profile}/#{testset}/#{product.name}"
                      }
                    end
                  }
              end
